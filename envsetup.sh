...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^aosp_") ; then
       FREAK_BUILD=$(echo -n $1 | sed -e 's/^aosp_//g')
    else
       FREAK_BUILD=
    fi
    export FREAK_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    FREAK_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/freak/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the freak model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch freak_$target-$variant
        fi
    fi
    return $?
}
